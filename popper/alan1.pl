...
%% #defined recall2/1.
%% #defined recall3/1.
%% #defined unsat_precon_pa_qab/2.
%% #defined unsat_postcon_pab_qb/2.
...
pr :-
    unique_pAb(P),
    body_literal(R,P,_,(_,B)),
    #count{A : body_literal(R,P,_,(A,B))} > 1.

pr :-
    unique_paB(P),
    body_literal(R,P,_,(A,_)),
    #count{B : body_literal(R,P,_,(A,B))} > 1.

pr :-
    unique_pAB(P),
    body_literal(R,P,_,(_,_)),
    #count{A,B : body_literal(R,P,_,(A,B))} > 1.

pr :-
    unique_pAbc(P),
    body_literal(R,P,_,(_,B,C)),
    #count{A : body_literal(R,P,_,(A,B,C))} > 1.

pr :-
    unique_paBc(P),
    body_literal(R,P,_,(A,_,C)),
    #count{B : body_literal(R,P,_,(A,B,C))} > 1.

pr :-
    unique_pabC(P),
    body_literal(R,P,_,(A,B,_)),
    #count{C : body_literal(R,P,_,(A,B,C))} > 1.

pr :-
    unique_pABc(P),
    body_literal(R,P,_,(_,_,C)),
    #count{A,B : body_literal(R,P,_,(A,B,C))} > 1.

pr :-
    unique_paBC(P),
    body_literal(R,P,_,(A,_,_)),
    #count{B,C : body_literal(R,P,_,(A,B,C))} > 1.

pr :-
    unique_pAbC(P),
    body_literal(R,P,_,(_,B,_)),
    #count{A,C : body_literal(R,P,_,(A,B,C))} > 1.

pr :-
    unique_pABC(P),
    body_literal(R,P,_,(_,_,_)),
    #count{A,B,C : body_literal(R,P,_,(A,B,C))} > 1.

pr :-
    unique_paBCD(P),
    body_literal(R,P,_,(A,_,_,_)),
    #count{B,C,D : body_literal(R,P,_,(A,B,C,D))} > 1.

pr :-
    unique_paBCD(P),
    body_literal(R,P,_,(A,B,_,_)),
    #count{C,D : body_literal(R,P,_,(A,B,C,D))} > 1.

pr :-
    asymmetric_pabc_pacb(P),
    body_literal(R,P,_,(A,B,C)),
    body_literal(R,P,_,(A,C,B)).

pr :-
    asymmetric_pabc_pcba(P),
    body_literal(R,P,_,(A,B,C)),
    body_literal(R,P,_,(C,B,A)).

pr :-
    asymmetric_pabc_pcba(P),
    body_literal(R,P,_,(A,B,C)),
    body_literal(R,P,_,(B,A,C)).

pr :-
    asymmetric(P),
    body_literal(R,P,_,(A,B)),
    body_literal(R,P,_,(B,A)).

pr :-
    asymmetric_pq(P,Q),
    body_literal(R,P,_,(A,B)),
    body_literal(R,Q,_,(B,A)).

pr :-
    antitransitive(P),
    body_literal(R,P,_,(A,B)),
    body_literal(R,P,_,(B,C)),
    body_literal(R,P,_,(A,C)).

pr :-
    antitriangular(P),
    body_literal(R,P,_,(A,B)),
    body_literal(R,P,_,(B,C)),
    body_literal(R,P,_,(C,A)).

pr :-
    unsat_pair_pa(P,Q),
    body_literal(R,P,_,Vars),
    body_literal(R,Q,_,Vars).
...
pr :-
    ...
    body_literal(R,P,_,(A,)),
    body_literal(R,Q,_,(A,_)).
pr :-
    unsat_postcon_pab_qb(P,Q),
    body_literal(R,P,_,(_,B)),
    body_literal(R,Q,_,(B,)).
...
%%     recall2(P),
%%     clause(R),
%%     #count{Vars : body_literal(R,P,_,Vars)} > 2.

%% pr :-
%%     recall3(P),
%%     clause(R),
%%     #count{Vars : body_literal(R,P,_,Vars)} > 3.
