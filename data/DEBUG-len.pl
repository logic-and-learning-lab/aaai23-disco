...
a:-
    forall(prop(Name,Args), writeln(Name-Args)), halt.

%% #count{Vars : holds(P,Vars), holds(Q,Vars)} == 0.
...
prop(unique_a_b,(P)):- body_pred(P,2), \+unique_a_b_(P).
...
unique_a_b_(P):-
    ...
    call(P,A,C),
    B\=C,!.
    %% holds(P,(A,_)), #count{B : holds(P,(A,B))} > 1.
